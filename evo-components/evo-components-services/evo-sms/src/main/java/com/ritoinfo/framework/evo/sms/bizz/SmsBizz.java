package com.ritoinfo.framework.evo.sms.bizz;

import com.ritoinfo.framework.evo.common.uitl.DateUtil;
import com.ritoinfo.framework.evo.sms.config.properties.SmsProperties;
import com.ritoinfo.framework.evo.sms.dto.SmsDto;
import com.ritoinfo.framework.evo.sms.exception.SmsException;
...
public class SmsBizz {
	...
	public void send(SmsDto smsDto) {
		if (smsProperties.isEnabled()) {
			String queryString = smsProperties.getAddress() +
					"?comCode=" + smsProperties.getComCode() +
					"&orgCode=" + smsProperties.getOrgCode() +
					"&certiNo=" + DateUtil.formatDatetimeCompact(DateUtil.now()) +
					"&businessNo=" + smsProperties.getBusinessNo() +
					"&businessTypeCode=" + smsProperties.getBusinessTypeCode() +
					"&channel=" + smsProperties.getChannel() +

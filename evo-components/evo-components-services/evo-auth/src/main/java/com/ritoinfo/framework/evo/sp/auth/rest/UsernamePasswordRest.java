...
import com.ritoinfo.framework.evo.sp.auth.bizz.UsernamePasswordBizz;
...
public class UsernamePasswordRest {
	...
	public ServiceResponse<String> login(@Validated @RequestBody UsernamePasswordParam usernamePasswordParam, HttpServletRequest request) {
		try {
			return ServiceResponse.ok(usernamePasswordBizz.login(usernamePasswordParam, HttpServletUtil.extractRemoteAddr(request)));

package com.github.framework.evo.common.exception;

/**
 * User: Kyll
 * Date: 2018-04-12 14:05
 */
public class StringOperateException extends BaseException {
	public StringOperateException(String message) {
		super(message, null, null);
	}

	public StringOperateException(String message, Object data) {
		super(message, data, null);
	}

	public StringOperateException(String message, Throwable throwable) {
		super(message, null, throwable);
	}

	public StringOperateException(String message, Object data, Throwable throwable) {
		super(message, data, throwable);
	}
}

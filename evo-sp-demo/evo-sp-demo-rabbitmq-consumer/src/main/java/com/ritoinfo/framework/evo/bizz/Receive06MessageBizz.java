...
@Slf4j
@Service
public class Receive06MessageBizz {
	...
	public void process() {
		...
		for (String routeKey : new String[]{"kern.*"}) {
			CommonUtil.queueBind(channel, queueName, EXCHANGE_NAME, routeKey);
		}

		log.info("等待接收消息");
		CommonUtil.basicConsume(channel, queueName, true, new WorkDefaultConsumer(channel, "Receive06"));
	}
}

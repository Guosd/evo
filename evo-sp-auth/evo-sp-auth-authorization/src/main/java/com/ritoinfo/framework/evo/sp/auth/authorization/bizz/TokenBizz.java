...
public class TokenBizz {
	...
	public String tryRefresh(String token) {
		return redisService.getString(redisAssist.generate(AuthConst.BF_OLD_TOKEN, token));
	}

	public String refresh(String token) {
		String refreshToken = redisService.getString(redisAssist.generate(AuthConst.BF_REFRESH_TOKEN, token));
		...
		if (VerifyResult.SUCCESS == verifyResult) {
			...
			redisService.set(redisAssist.generate(AuthConst.BF_OLD_TOKEN, token), newToken, 30 * 1000L);
			redisService.delete(redisAssist.generate(AuthConst.BF_REFRESH_TOKEN, token));
	...
	private String createToken(UserContext userContext) {
		String onlineKey = redisAssist.generate(AuthConst.BF_ONLINE, userContext.getId());
		...
		if (StringUtil.isNotBlank(onlineToken)) {
			...
			redisService.delete(redisAssist.generate(AuthConst.BF_ACCESS_TOKEN, onlineToken));
			redisService.delete(redisAssist.generate(AuthConst.BF_REFRESH_TOKEN, onlineToken));
			redisService.delete(redisAssist.generate(AuthConst.BF_OLD_TOKEN, onlineToken));
		}
		...
		// 设置缓存
		redisService.set(redisAssist.generate(AuthConst.BF_ACCESS_TOKEN, accessToken), userContext, accessTokenExpiration);
		redisService.set(redisAssist.generate(AuthConst.BF_REFRESH_TOKEN, accessToken), refreshToken, refreshTokenExpiration);

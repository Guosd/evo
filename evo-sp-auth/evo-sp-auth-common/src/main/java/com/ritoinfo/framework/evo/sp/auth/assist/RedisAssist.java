...
public class RedisAssist {
	...
	public void saveVerifyCode(String bizzFlag, String mobileNumber, String verifyCode) {
		redisService.set(generate(AuthConst.VERIFY_CODE_PREFIX + bizzFlag, mobileNumber), verifyCode, 1000L * 60 * authProperties.getVerifyCode().getExpirationTime());
	}

	public String getVerifyCode(String bizzFlag, String mobileNumber) {
		return redisService.getString(generate(AuthConst.VERIFY_CODE_PREFIX + bizzFlag, mobileNumber));
	}

	public void deleteVerifyCode(String mobileNumber) {
		redisService.delete(generate(AuthConst.VERIFY_CODE_PREFIX + Const.VERIFY_CODE_SIGN_IN, mobileNumber));
		redisService.delete(generate(AuthConst.VERIFY_CODE_PREFIX + Const.VERIFY_CODE_SIGN_UP, mobileNumber));
	}

	public boolean existAccessToken(String accessToken) {
		return redisService.exist(generate(AuthConst.BF_ACCESS_TOKEN, accessToken));
	}
}

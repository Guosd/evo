...
import com.ritoinfo.framework.evo.sp.auth.authorization.api.PermissionApi;
import com.ritoinfo.framework.evo.sp.auth.authorization.api.TokenApi;
...
public class AuthFilter extends ZuulFilter {
	...
	public Object run() {
		...
		for (String path : authProperties.getPath().getExcludes()) {
		...
		if (exclude) {
			...
			String accessToken = HttpServletRequestUtil.extractAccessToken(request);
	...
	private void process401(RequestContext requestContext) {
		...
		if (NetUtil.isRequestFromBrowser(requestContext.getRequest())) {
			requestContext.setResponseStatusCode(HttpStatus.SC_TEMPORARY_REDIRECT);

			String login = authProperties.getPath().getLogin();

			requestContext.put(FilterConstants.FORWARD_TO_KEY, login);
			try {
				requestContext.getResponse().sendRedirect(login);
			} catch (IOException e) {
